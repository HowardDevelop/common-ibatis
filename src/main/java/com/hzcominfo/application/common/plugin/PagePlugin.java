package com.hzcominfo.application.common.plugin;
...
import com.hzcominfo.application.database.ibatis.plugin.PluginInvocationHandler;
...
public class PagePlugin implements Interceptor {
                ...
                if (mappedStatement.getId().indexOf(Constant.DOT_BASEMAPPER_DOT) > -1) {
                    ...
                    if (parameterObject instanceof FormMap) {
                        ...
                        if (map.containsKey("list")) {
                            ...
                            String sql = PluginInvocationHandler
                    ...
                    String sql = PluginInvocationHandler.joinSql(connection, mappedStatement, boundSql, formMap, null);
                    ReflectHelper.setValueByFieldName(boundSql, "sql", sql);
                    return ivk.proceed();
                }
     ...
     * ly_userinfo u where u.id=userId) loginName, (SELECT userName from
     * ly_userinfo u where u.id=userId) userName, sum(ddd) ss from article
     * 兼容以上子查询 //去除sql ..from 前面的字符。考虑 aafrom fromdd 等等情况
     */
    public static String suffixStr(String toSql) {
        toSql = toSql.replaceAll("\\(\\s+[S|s][E|e][L|l][E|e][C|c][T|t]", "(select");
        int sun = toSql.toLowerCase().indexOf("from");
        ...
        if (f1.trim().isEmpty() && f2.trim().isEmpty()) {// 判断第一个from的前后是否为空
            ...
            int s0 = s1.indexOf("(");
            if (s0 > -1) {
                int se1 = s1.toLowerCase().indexOf("select");
                if (s0 < se1) {
                    if (se1 > -1) {
                ...
                int se2 = s1.toLowerCase().indexOf("(select");
                if (se2 > -1) {
    ...
    private static String removeOrderBys(String toSql) {
        int sun = toSql.toLowerCase().indexOf("order");
        if (sun > -1) {
            String f1 = toSql.substring(sun - 1, sun);
    ...
    public static String buildPageSqlForSQLServer2008Dialect(String sql, PageView page) {
        ...
        String sqlbuild = dialect.getLimitString(sql, (page.getPageNow() - 1) * page.getPageSize(),
                page.getPageSize());
        return sqlbuild;
    }
