...
public class SQLServer2008Dialect {
    ...
    public String getTotalString(String sql) {
        ...
        sb.insert(0, "WITH query AS (").append(") SELECT count(*) total FROM query ");
        return sb.toString();
    }
     ...
     */
    private String getLimitString(String querySqlString, int offset, int limit, boolean hasOffset) {
        ...
        sb.append("WHERE __liud_row_nr__ BETWEEN ").append(offset + 1).append(" AND ").append(offset + limit);
        return sb.toString();
    }
    ...
    protected static void insertRowNumberFunction(StringBuilder sql, CharSequence orderby) {
        ...
        sql.insert(selectEndIndex, " ROW_NUMBER() OVER (" + orderby + ") as __liud_row_nr__,");
    }
