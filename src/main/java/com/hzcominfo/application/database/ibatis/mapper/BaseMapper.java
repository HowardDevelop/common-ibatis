package com.hzcominfo.application.database.ibatis.mapper;

import java.util.List;

/**
 * 已经实现基本的 增,删,改,查接口,不需要重复写 所有mapper都继承这个BaseMapper
 */
public interface BaseMapper {
     ...
     * 8：追加或和非的查询：formMap.put('where"," and (id=XX or name= XX)") <br/>
     * 
     * @return <T> List<T>
     */
     ...
     * 4：兼容排序查询 : formMap.put("orderby","order by id desc");
     * 
     * @return <T> List<T>
     */
     ...
     * @return List<Map<?, ?>>
     */
    public <T> List<T> initTableFieldForMysql(T formMap);
     ...
     * @return List<Map<?, ?>>
     */
    public <T> List<T> initTableFieldForOracle(T formMap);
